            ...
            messages: [
              {
                ...
                content: [
                  {
                    type: 'text',
                    text: 'Analise esta imagem de ETIQUETA DE GÔNDOLA de supermercado. As etiquetas de gôndola são as etiquetas fixadas nas prateleiras que identificam os produtos e seus preços. FOQUE PRINCIPALMENTE nos dados impressos nas etiquetas de gôndola. Extraia todos os produtos visíveis com seus nomes, marcas, preços, EAN e tamanho/unidade das ETIQUETAS. IMPORTANTE: Diferencie entre preços de ATACADO (necessita quantidade mínima) e VAREJO (preço unitário). IGNORE COMPLETAMENTE textos informativos como "Preço por kg", "Preço por litro", ou qualquer texto que seja apenas descritivo/comparativo e não o preço real do produto. Capture APENAS o preço real do produto mostrado na ETIQUETA DE GÔNDOLA. Se houver múltiplos preços, pegue sempre o preço da unidade completa do produto. Retorne no formato JSON com array "items", onde cada item tem: "name" (nome do produto), "brand" (marca), "retail_price" (preço varejo em número), "wholesale_price" (preço atacado em número ou null), "min_wholesale_qty" (quantidade mínima para atacado ou null), "ean" (código de barras EAN de 13 ou 8 dígitos, apenas números), "unit_size" (tamanho/unidade como "1kg", "500ml", "2L", etc.), "confidence" (confiança de 0 a 1). Se não conseguir identificar algum campo, use null. IGNORE outros valores que não sejam atacado ou varejo.'
                  },
                  {
                    type: 'image_url',
                    image_url: {
